...
namespace FractureDestruction
{
    public class FractureThis : MonoBehaviour
    {
        ...
        /// <returns>Always 2 meshes, [0] is the one containing vertexIndexesToSplit</returns>
        private List<Mesh> FN_splitMesh(HashSet<int> vertexIndexesToSplit, Mesh orginalMesh, bool doBones)
        {
            //setup
            ...
            BoneWeight[] bones = orginalMesh.boneWeights;
            ...
            for (int i = 0; i < tris.Length; i += 3)
            {
                ...
                {
                    splitTriA.Add(FN_getIndexOfVertex(verts[tris[i]], nors[tris[i]], uvs[tris[i]], bones[tris[i]], true));
                    splitTriA.Add(FN_getIndexOfVertex(verts[tris[i + 1]], nors[tris[i + 1]], uvs[tris[i + 1]], bones[tris[i + 1]], true));
                    splitTriA.Add(FN_getIndexOfVertex(verts[tris[i + 2]], nors[tris[i + 2]], uvs[tris[i + 2]], bones[tris[i + 2]], true));
                }
                else
                {
                    splitTriB.Add(FN_getIndexOfVertex(verts[tris[i]], nors[tris[i]], uvs[tris[i]], bones[tris[i]], false));
                    splitTriB.Add(FN_getIndexOfVertex(verts[tris[i + 1]], nors[tris[i + 1]], uvs[tris[i + 1]], bones[tris[i + 1]], false));
                    splitTriB.Add(FN_getIndexOfVertex(verts[tris[i + 2]], nors[tris[i + 2]], uvs[tris[i + 2]], bones[tris[i + 2]], false));
                }
            }
            ...
            int FN_getIndexOfVertex(Vector3 vPos, Vector3 vNormal, Vector2 vUv, BoneWeight vBone, bool checkSplitA)
            {
                if (checkSplitA == true)
                {
