...
| **MaterialStrength**   | The maximum stress that a node connection can withstand. |
...
| **NodeStrength**       | The maximum stress a node connection can endure before breaking. |
...
The general concept is to determine the force required to move each node so the node structure maintains the same shape if a given set of nodes are forced to move.

### 3.1. Structure Initialization
Start by creating an array of nodes in 3D space and connect all neighboring nodes, so a node structure is formed. A node usually represents a chunk of a mesh, the AABB of the chunk can be used to determine if two nodes are neighbors.
...
3.3.2. **Calculate Actuall Stress**:
   ...
   - Calculate the `total mass (M)` by summing the `NodeMass` from each `Source`.
   - For each node in `Source`, set `NodeStress = min(NodeMass * Velocity, Velocity * Mass * (NodeMass / M))`.
   - Set `S = 0`.
   - Propagate Stress:
      - For each node where `NodeStep == S`, get all neighbors where `NodeStep > S` (denoted as `N`).
      - If `(NodeDamage * MaterialDamageAccumulation) + (NodeStress / max(N count, 1)) > NodeDamage`, set `NodeDamage` to `(NodeDamage * MaterialDamageAccumulation) + (NodeStress / max(N count, 1))`.
      - Calculate the `total strength (T)` and `total mass (M)` by summing the `NodeStrength` and `NodeMass` from each `N`.
      - Clamp `T` and `M` so `T` is higher than `NodeStrenght` and `M` is higher than `NodeMass`
